...
import {
  ...
  AlertTriangle, Clock, Calendar, TrendingDown,
  Download, Eye,
} from 'lucide-react'
...
function DueBadge({ tx }: { tx: Transaction }) {
  ...
  return <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">{fmtDate(tx.dueDate)}</span>
}
...
function exportCsv(transactions: Transaction[]) {
  const header = '"Data Vencimento","DescriÃ§Ã£o","Fornecedor","Categoria","Conta","Valor","Status"'
  ...
  const a    = document.createElement('a'); a.href = url; a.download = 'contas-a-pagar.csv'; a.click()
    ...
    async function loadMeta() {
      try {
        const [catRes, bankRes] = await Promise.all([
          fetch(`${API}/api/financial/categories?type=EXPENSE`, { headers: headers() }),
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Gerencie todas as despesas e obrigaÃ§Ãµes da empresa</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <SummaryCard loading={loading} title="Vencidas" value={summaryOverdue} count={summaryCountOv}
          label="Contas em atraso" icon={AlertTriangle} iconCls="text-red-500" iconBgCls="bg-red-100" />
        <SummaryCard loading={loading} title="Vencem hoje" value={summaryDueToday} count={summaryCountToday}
          label="Vencimento no dia" icon={Clock} iconCls="text-orange-500" iconBgCls="bg-orange-100" />
        <SummaryCard loading={loading} title="PrÃ³ximos 7 dias" value={summaryDue7} count={summaryCountDue7}
          label="A vencer em breve" icon={Calendar} iconCls="text-amber-600" iconBgCls="bg-amber-100" />
        <SummaryCard loading={loading} title="Total pendente" value={summaryTotal} count={summaryCountTotal}
          label="Total de despesas abertas" icon={TrendingDown} iconCls="text-violet-600" iconBgCls="bg-violet-100" />
      </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-5 py-3 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide whitespace-nowrap hidden lg:table-cell">Fornecedor</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-5 py-16 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm font-medium text-gray-500">Nenhuma despesa encontrada</p>
                      <p className="text-xs text-gray-400">Altere os filtros ou crie uma nova despesa.</p>
                    </div>
                  </td>
                </tr>
              ) : transactions.map((tx) => {
                ...
                return (
                  <tr key={tx.id} className={`hover:bg-gray-50 transition-colors ${rowCls}`}>
                    ...
                    <td className="px-5 py-3 text-sm font-bold tabular-nums text-red-600 whitespace-nowrap">
                      {fmt(tx.netAmount)}
                    </td>
